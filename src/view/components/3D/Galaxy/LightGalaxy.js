...
export default function LightGalaxy({ position, config }) {
  ...
  const uniforms = {
    ...
    uniColor: { value: rgbObject2ThreeColor(galaxyColorControls.uniColor) },
    uniColor2: { value: rgbObject2ThreeColor(galaxyColorControls.uniColor2) },
  ...
  const parameters = useControls({
    Galaxy: folder({
      count: { min: 100, max: 90000, value: 36000, step: 100 },
  ...
  useEffect(generateGalaxy, [parameters])
  ...
  function generateGalaxy() {
    ...
    for (let i = 0; i < parameters.count; i++) {
      ...
      const randomX =
        Math.pow(Math.random(), Number(parameters.randomnessPower)) *
        (Math.random() < 0.5 ? 1 : -1) *
